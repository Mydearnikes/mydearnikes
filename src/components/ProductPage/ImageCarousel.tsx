...
export default function ImageCarousel({ product, selectedColor }: ImageCarouselProps) {
  ...
  const [scale, setScale] = useState(1);
  const [isDragging, setIsDragging] = useState(false);
  const [displayImages, setDisplayImages] = useState<any[]>([]); // ✅ ADD THIS
  ...
  // ✅ ADD THIS - Get images based on selected color
  useEffect(() => {
    const getImagesByColor = () => {
      ...
      // If no color selected, return all product images
      if (!selectedColor) {
      ...
      // If variant has an image, show it first
    ...
    const newImages = getImagesByColor();
    setDisplayImages(newImages);
    setActiveIndex(0); // Reset to first image when color changes
    ...
    <>
      ...
      >
        <Swiper
          ...
          onSlideChange={(swiper) => setActiveIndex(swiper.activeIndex)}
          className="h-[50vh] w-full md:h-[110vh] lg:h-[93vh] rounded-none"
          key={selectedColor} // ✅ ADD THIS - Forces re-render on color change
        >
          {/* ✅ CHANGE: Use displayImages instead of images */}
        ...
        <motion.div
          ...
          className="absolute bottom-2 left-1/2 -translate-x-1/2 z-20"
        >
          {/* ✅ CHANGE: Use displayImages.length */}
          <CustomBullet total={displayImages.length} activeIndex={activeIndex} />
        </motion.div>
      </motion.div>

      {/* ✅ KEEP YOUR ENTIRE LIGHTBOX CODE - Just update image references */}
      <AnimatePresence>
        {isLightboxOpen && (
          <motion.div
            initial={{ opacity: 0 }}
            ...
            className="fixed inset-0 z-50"
          >
            {/* ... All your existing lightbox code ... */}
            {/* Just make sure to use displayImages instead of images everywhere in lightbox */}
          </motion.div>
        )}
      </AnimatePresence>
    </>
